...
>> 송신 호스트와 수신 호스트가 패킷 교환 네트워크에서 정보를 주고받는 데 사용되는 프로토콜이다. 쉽게 말해, 인터넷 상에서 데이터를 주고받는 데 사용되는 주소 체계라 보면 된다. 자세하게는, 호스트의 주소 지정과 패킷 분할 및 조립 기능을 담당한다. 버전은 IPv4와 IPv6이 있으며 특징으로는 비신뢰성, 비연결성, 프로세스 구분x 등이 있다.
> ## IPv4
