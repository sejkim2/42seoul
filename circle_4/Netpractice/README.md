...
## IP 할당
>> 호스트가 IP 주소를 할당받을 때 사용하는 방법은 고정 IP 할당과 유동 IP 할당이 있다. 고정 IP는 호스트의 주소를 네트워크 관리자가 고정 할당하는 방법으로 항상 같은 주소를 사용하기 때문에 접근성이 좋고 주소 관리가 편하지만 주소 공간이 낭비되고 사용 유연성이 떨어진다는 단점이 있다. (만약 주소가 변경되면 해당 주소를 사용하는 모든 장치에 수정을 해야되므로) 유동 IP는 DHCP 서버에서 임시 IP를 호스트에게 할당하며 임대기간 동안 해당 주소를 사용하도록 한다. 자원을 사용 및 반납하므로 주소 공간을 절약할 수 있지만 동일한 주소를 갖는 것이 아니기 때문에 고정 IP에 비해 접근성이 떨어진다.

## DHCP(Dynamic Host Configuration Protocol)
>> [DHCP 클라이언트-서버 상호작용 사진]
>> 네트워크 관리자가 매번 호스트의 주소를 수동으로 할당하는 것이 실용적이지 않기 때문에 (IP 주소가 익숙하지 않은 사용자들을 위해서라도) 보다 쉽고 유연한 IP 할당을 위해 DHCP가 사용된다.
> ## DHCP 서버 발견 : 새로 도착한 클라이언트(호스트)는 상호작용할 서버(DHCP)를 발견한다. 이때 클라이언트는 DHCP 발견 메시지를 보내는데 자신의 IP 주소 및 서버 주소를 모르므로 해당 서브넷 내 모든 노드로 브로드캐스트한다.
> ## DHCP 서버 제공 : DHCP 발견 메시지를 받은 서버는 IP 주소, 주소 임대 기간 등이 포함되어 있는 DHCP 제공 메시지를 클라이언트로 응답한다. 이때 역시 서버는 클라이언트로 제공 메시지를 브로드캐스트한다. (일반적으로 서브넷에는 여러 DHCP 서버가 존재하기 때문에 클라이언트는 여러 제공 메시지를 받고 그 중 최적의 DHCP 서버를 선택하기 위해 모든 서버로부터 제공 메시지를 받는다.)
> ## DHCP 서버 요청 : 클라이언트는 제공받은 IP 주소를 사용하겠다는 요청을 서버에게 보낸다.
> ## DHCP 서버 ACK : 클라이언트의 IP 사용을 승인하는 ACK가 도착하면 상호작용은 종료되고 클라이언트는 임대 기간 동안 해당 IP를 사용할 수 있다.
